...
function Navbar() {
  ...
  const [themeDropdown, setThemeDropdown] = useState(true);
  ...
  const handleClick = (e) => {
    ...
    if (themeDropdown && (!isTheme || !e.target.classList.contains('theme'))) {
        offThemeDropdown();
      }
  };
  ...
  return (
    <header className=" bg-dark-primary h-[1.5cm] w-full md:pt-[10px] sticky top-0">
      <nav className=" h-[1.4cm] w-full flex flex-row justify-end items-center gap-8 px-3 md:px-10 relative">
        ...
        {profileDropdown && (
          <div className="mt-[1.5cm] rounded-xl h-[6cm] w-[4.5cm] bg-dark-overlay shadow-[10px] absolute top-0 right-0 mr-3 p-3 flex flex-col gap-4">
            ...
            <button className="absolute bottom-3 bg-dark-text-active rounded-md text-dark-text-primary grid grid-cols-5 py-1 px-1">
        ...
        {themeDropdown && (
          <div className="mt-[1.5cm] rounded-xl h-[6cm] w-[5cm] bg-dark-overlay shadow-[10px] absolute top-0 right-0 mr-[4rem] md:mr-[7rem] p-3 flex flex-row flex-wrap gap-4 ">
            <ThemeButtons
              backgound={"bg-dark-primary"}
              thirty={"bg-dark-text-active"}
              ten={"bg-dark-overlay"}
